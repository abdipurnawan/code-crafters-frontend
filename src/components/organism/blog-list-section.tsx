...
const BlogListSection = ({ blogs, tags }: BlogListSectionProps) => {
  // states
  // const tags = useRecoilValue(tagsDataState);
  const [clientBlogs, setClientBlogs] = useRecoilState(blogListDataState);
  ...
  const blogsQuery = useQuery(['blogs', activeTag, activeUrl], () => fetchBlogs(), {
    ...
    onSuccess: (data: PaginationModel<BlogModel[]>) => {
      setClientBlogs(data.data as PaginationModel<BlogModel[]>);
    }
  });
  ...
  if (clientBlogs?.data && clientBlogs?.data?.length > 0) {
    return (
          ...
          <div className='bg-white p-6 rounded-3xl'>
            ...
            <div className='grid grid-cols-1 gap-16 md:grid-cols-2 md:p-8 lg:grid-cols-3 lg:p-14'>
              ...
              {!blogsQuery.isLoading &&
                ...
                clientBlogs?.data?.map((blog, index) => <BlogItem key={index} blog={blog} />)}
            </div>
